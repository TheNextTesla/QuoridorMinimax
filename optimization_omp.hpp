...
float alphabeta(Board board, unsigned int depth, float alpha, float beta, bool player1, table_map& table);
float negamax(Board board, unsigned int depth, float alpha, float beta, bool player1);
...
Move minimax_parallel_helper(Board board, unsigned long ideal_depth, bool player1)
{
    ...
    #pragma omp parallel
    {
        ...
        for (unsigned int i = 0; i < first_level_moves.size(); i++)
        {
            ...
            // float score = negamax(move_board, depth-1, -beta, -alpha, !player1);
...
float alphabeta(Board board, unsigned int depth, float alpha, float beta, bool player1, table_map& table)
{
    ...
    if (player1)
    {
        ...
        for (Move move : valid_moves)
        {
            ...
            value = std::max(value, alphabeta(child, depth-1, alpha, beta, !player1, table));
    ...
    else
    {
        ...
        for (Move move : valid_moves)
        {
            Board child = board;
            child.apply_move(move);
            value = std::min(value, alphabeta(child, depth-1, alpha, beta, !player1, table));
...
//
//     if (depth == 0)
//         return (player1 ? 1 : -1) * calculate_heurisitic(board, true);
//
//     std::vector<Move> valid_moves = generate_valid_moves(board, player1);
//     // Order Moves
...
//         Board child = board;
//         child.apply_move(move);
//         value = std::max(value, -negamax(child, depth - 1, -beta, -alpha, !player1));
//         alpha = std::max(alpha, value);
//         if (alpha >= beta)
//             break;
//     }
